...
uint8_t mem_access(vaddr_t addr, char access, uint8_t data) {
    ...
    /* Either read or write the data to the physical address
       depending on 'rw' */
    pte_t * virtual = (pte_t *)(mem + PTBR * PAGE_SIZE) + vaddr_vpn(addr);
    if (!virtual->valid) {
        page_fault(addr);
    }
    frame_table[virtual->pfn].referenced = 1;

    paddr_t physical = (paddr_t)(virtual->pfn << OFFSET_LEN) | vaddr_offset(addr);
    stats.accesses++;
    if (access == 'r') {
        return mem[physical];
    }
